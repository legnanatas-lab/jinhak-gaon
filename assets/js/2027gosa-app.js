...
let REG_ORDER, REGIONS, initRegions, renderFilterUI, resetFilters;
let DAYS_ALL;
let evItemHTML, evDetailHTML;
let syncHash, renderCal, renderDay;
let renderUni, renderUniDetail;
let PKC, PKN, pickEvents, renderCounsel, renderCsRes, renderCsSheet;
let SORTS, renderList, exportCSV;

function initializeGosa() {
  DB = window.__GAONGIL_GOSA_2027_DB__;
  if (!DB) return false;

  /* ───────── 데이터 전개 ───────── */
  E = DB.events.map((a, i) => ({ i, u: a[0], t: a[1], st: a[2], d1: a[3], d2: a[4], p: a[5], m: a[6], app: a[7], ann: a[8], os: a[9], c: a[10] }));
  UNIS = DB.unis.map((a, i) => ({ i, full: a[0], short: a[1], region: a[2], multi: a[3], disp: a[3] ? a[0] : a[1] }));
  TY = ["논술", "면접", "실기"];
  CATS = ["학생부교과", "학생부종합", "논술위주", "실기/실적위주", "기타"];
  CAT_OF = { "학생부위주(교과)": 0, "학생부위주(종합)": 1, "논술위주": 2, "실기/실적위주": 3 };
  PATHS = DB.paths.map((p, i) => {
    const segs = p.split(" > ");
  ...
  Y = DB.meta.year;
  SUN = DB.meta.suneung;
  HOL = DB.meta.holidays;
  WD = ["일", "월", "화", "수", "목", "금", "토"];
  dObj = n => new Date(Y, (n / 100 | 0) - 1, n % 100);
  dow = n => dObj(n).getDay();
  fmtMD = n => `${n / 100 | 0}/${n % 100}`;
  fmtMDW = n => `${fmtMD(n)}(${WD[dow(n)]})`;
  nextDay = n => { const d = dObj(n); d.setDate(d.getDate() + 1); return (d.getMonth() + 1) * 100 + d.getDate(); };
  eachDay = (a, b, f) => { let c = a, g = 0; while (c <= b && g++ < 200) { f(c); c = nextDay(c); } };
  isHol = n => !!HOL[n];
  csatDiff = n => Math.round((dObj(n) - dObj(SUN)) / 864e5);
  csatLabel = n => { const d = csatDiff(n); return d === 0 ? "수능일" : d < 0 ? `수능 D${d}` : `수능 D+${d}`; };
  ...
  /* ───────── 필터 ───────── */
  nq = s => s.replace(/\s+/g, "").toLowerCase();
  pass = function(e) {
  ...
  saveCS = () => { try { localStorage.setItem("gosa2027_counsel", JSON.stringify(CS)); } catch (e) { } };
  curStu = () => CS.students[CS.cur];
  linkedSt2 = function(e) {
    if (e.st !== 1) return [];
  ...
  update = function() { refilter(); renderFilterUI(); renderView(); };

  /* ───────── 필터바 UI ───────── */
  REG_ORDER = ["서울", "인천", "경기", "강원", "대전", "세종", "충남", "충북", "광주", "전남", "전북", "대구", "경북", "부산", "울산", "경남", "제주"];
  REGIONS = REG_ORDER.filter(r => UNIS.some(u => u.region === r));
  ...
  /* ───────── 1. 월별 캘린더 ───────── */
  syncHash = function() { if (S.view === "cal") { hashLock = true; location.hash = "cal/" + S.month; setTimeout(() => hashLock = false, 0); } };
  renderCal = function() {
  ...
  /* ───────── 3. 상담 모드 ───────── */
  PKC = ["#C2491F", "#5B54C7", "#15815F", "#B5851B", "#B23030", "#1B5FA8", "#6B6960", "#8A4FBF"];
  PKN = ["①", "②", "③", "④", "⑤", "⑥", "⑦", "⑧"];
  pickEvents = function(pk) { const e = E[pk.e]; return [e, ...linkedSt2(e)]; };
  renderCounsel = function() {
    $("#csStu").innerHTML = CS.students.map((s, i) =>
      `<option value="${i}" ${i === CS.cur ? "selected" : ""}>${esc(s.name)} (${s.picks.length})</option>`).join("");
    renderCsRes();
    renderCsSheet();
  };
  renderCsRes = function() {
    const box = $("#csRes");
    if (S.csUni !== null) {
  ...
  /* ───────── 4. 전체 목록 ───────── */
  SORTS = { d: (a, b) => a.d1 - b.d1, uni: (a, b) => UNIS[a.u].disp.localeCompare(UNIS[b.u].disp, "ko"), reg: (a, b) => UNIS[a.u].region.localeCompare(UNIS[b.u].region, "ko"), ty: (a, b) => a.t - b.t, cat: (a, b) => PATHS[a.p].cat - PATHS[b.p].cat, mc: (a, b) => a.m.length - b.m.length };
  renderList = function() {
    const [k, dir] = S.sort;
    ...
    $("#lstTbl").innerHTML = `<thead><tr>${th("d", "고사일")}<th>수능</th>${th("ty", "uni", "reg", "cat", "모집단위", "링크")}<th>유형</th><th>대학</th><th>지역</th><th>전형구분</th><th>전형명</th><th>모집단위</th><th class="noprint">링크</th></tr></thead><tbody>`
...
(function bootApp() {
  if (!initializeGosa()) {
    const timer = setInterval(() => {
      if (initializeGosa()) clearInterval(timer);
    }, 10);

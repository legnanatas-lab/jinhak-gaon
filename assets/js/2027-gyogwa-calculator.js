...
function calcManualRules(u) {
  ...
  return Object.values(u.manualRules).map(rule => {
    ...
    if(rule.formula==='yonsei2027'){
      ...
      return{label:rule.label,score,maxScore:rule.maxScore,avgGrade:'등급50%+Z점수50%',careerUsed:careerList.length,desc:`공통 ${c.toFixed(3)}×30% + 일반 ${g.toFixed(3)}×50% + 진로 ${v.toFixed(3)}(A20/B15/C10) − 기타과목 감점 ${deduct.toFixed(3)}${legacyRecovered?' · 구버전 과목구분 자동복구':''}`};
    }

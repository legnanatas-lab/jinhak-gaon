...
function calcManualRules(u) {
  ...
  return Object.values(u.manualRules).map(rule => {
    ...
    if(rule.formula==='skku2027'){
      const period=s=>s.grade<3||(s.grade===3&&s.sem===1),calc=(areas,points)=>{const list=common.filter(s=>period(s)&&areas.includes(s.area));if(!list.length)return{point:null,grade:null,count:0};const units=list.reduce((a,s)=>a+credit(s),0),point=list.reduce((a,s)=>a+points[s.rank-1]*credit(s),0)/units;let grade=9;for(let i=0;i<8;i++){if(point<=points[i]&&point>=points[i+1]){grade=(i+1)+(points[i]-point)/(points[i]-points[i+1]);break;}}return{point,grade,count:list.length};},a=calc(rule.aAreas,rule.aPoints),b=calc(rule.bAreas,rule.bPoints);if(a.point===null)return{label:rule.label,unavailable:true,reason:'A군(국어·수학·영어·한국사·사회·과학) 공통/일반선택 석차등급을 입력해 주세요.'};const score=a.point*7+(b.point??0),comparisonGrade=b.grade===null?a.grade:(a.grade*7+b.grade)/8;
      return{label:rule.label,score,maxScore:rule.maxScore,comparisonGrade,avgGrade:`A군 ${a.grade.toFixed(2)} · ${b.grade===null?'B군 미입력':`B군 ${b.grade.toFixed(2)}`}`,careerUsed:0,desc:`A군 ${a.count}과목 이수단위 가중점수 ${a.point.toFixed(3)}×7${b.point===null?' + B군 미입력':` + B군 ${b.count}과목 ${b.point.toFixed(3)}×1`} · 공식 A/B군 등급점수표 역산 비교환산 ${comparisonGrade.toFixed(3)}등급 · 진로는 정성200점에서만 평가`};
    }
